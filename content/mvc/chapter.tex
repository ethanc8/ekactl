...
\begin{procedure}[Equation of plane containing three points]
  If $A, B, C \in \Real^3$ are points on our plane, then we can find the normal vector by performing $n = \vec{AB} \crossp \vec{AC} = (B - A) \crossp (C - A)$ (since $\vec{AB}$ and $\vec{AC}$ are vectors on the plane).
\end{procedure}
...
\begin{procedure}[Distance between point and line]
  ...
  \[
    |\overrightarrow{AP} - \proj_L \overrightarrow{AP}| = \vec{P}
  \]
\end{procedure}
